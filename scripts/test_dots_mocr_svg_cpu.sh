...
CKPT=${CKPT:-${DOTS_MOCR_SVG_CKPT:-/mnt/nvme/huggingface/models--rednote-hilab--dots.mocr-svg/snapshots/main}}
MAX_NEW_TOKENS=${MAX_NEW_TOKENS:-256}
MAX_PIXELS=${MAX_PIXELS:-1000000}
RESULT_DIR=${RESULT_DIR:-/workspace/ocr/output/benchmarks/dots_mocr_svg_cpu}
...
exec "$RUN" cpu python3 -m dots_mocr.benchmark \
    --ckpt "$CKPT" \
    --input "$INPUT" \
    --prompt "${PROMPT:-prompt_image_to_svg}" \
    --device cpu \
    --dtype "${DTYPE:-float32}" \
