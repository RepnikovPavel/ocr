...
export OCRC_SERVER=http://127.0.0.1:8601

ocrc parse paper.pdf              # распарсить, дождаться, распаковать
ocrc queue                        # кто уже в очереди и под каким именем
ocrc watch                        # тот же поток событий без опроса
ocrc search "attention"           # полнотекстовый поиск по разобранному
```

Под ним — `/api/v1` на том же порту, что и UI:

| вызов | что делает |
|---|---|
| `POST /api/v1/documents` | подать файл; отвечает `queued` или `cached` |
| `GET /api/v1/documents/{sha256}` | статус, прогресс, токены, секунды |
| `GET /api/v1/documents/{sha256}/bundle` | результат одним ZIP: `document.md` + `images/` + `layout/` |
| `GET /api/v1/documents?q=` | полнотекстовый поиск (SQLite FTS5) |
| `GET /api/v1/queue` | очередь с именами агентов |
| `GET /api/v1/events` | Server-Sent Events по очереди |
| `GET /api/v1/stats` | размер базы и доля переиспользования |

**Дедуп по содержимому.** Результат ключуется тройкой: SHA-256 загруженных байт,
режим промпта, набор страниц — все три меняют ответ. Замерено на статье с arxiv:
первый разбор 27.6 с, тот же запрос потом — **0.17 с**. Поэтому агентам выгоднее
переспрашивать сервис, чем кэшировать у себя, а несколько агентов, запросивших
одну статью, платят за один разбор.

**Простой освобождает карту.** Если агенты ничего не просят `DEMO_IDLE_UNLOAD_S`
секунд (180 по умолчанию), модель выгружается: для vLLM это sleep mode, память
возвращается (замерено 9.4 → 2.0 ГиБ). Следующая заявка будит автоматически.

**База разобранных документов** живёт в том же SQLite, что и очередь, — деплой
не усложняется, бэкап остаётся одним файлом.
...
### 0. На сервере одной командой (без принятия решений)

`scripts/bootstrap.sh` сам находит чекпойнт, определяет версию CUDA-драйвера и
подбирает docker-образы, которые на ней реально стартуют (сборка cu13x
молча отказывается на драйвере CUDA 12.x — именно это обычно и съедает время при
развёртывании), собирает недостающие образы, поднимает `deploy_server.sh`,
проверяет через `doctor.sh` и печатает команду для коллег:

```bash
bash scripts/bootstrap.sh                  # 127.0.0.1, доступ через SSH-туннель
bash scripts/bootstrap.sh --bind lan       # 0.0.0.0, доверенная сеть → одна строка для коллег
bash scripts/bootstrap.sh --dry-run        # только preflight + автоопределение, без деплоя
bash scripts/bootstrap.sh --down           # остановить
```

Override при желании: `CKPT=`, `VLLM_GPU=`, `DEMO_PORT=`/`VLLM_PORT=`,
`FORCE_VLLM_IMAGE=`/`FORCE_DEMO_IMAGE=`. Это и есть рецепт 3b под капотом —
можно сразу начинать отсюда и не читать остальные, если цель — поднять сервис.

> **Кэш разобранных документов уже встроен** (`demo/docstore.py`, дедуп по
> SHA-256 + режиму + страницам в SQLite) и поднимается автоматически с
> demo-контейнером. Отдельный кэш / SeaweedFS разворачивать не нужно — его не
> должно быть; повторный запрос того же документа отдаётся как lookup (~0.1 с).
...
### 3b. На сервере, полный сервис для агентов (vLLM + API)

Рецепт 3 поднимает только демку с моделью в процессе. Если нужен агентский API
на движке vLLM — два сервиса через compose:

```bash
# один раз: собрать образ vLLM на сервере
docker build -f docker/Dockerfile.vllm -t dots-mocr:vllm docker/

CKPT=/path/to/snapshot STATE=/path/to/state \
VLLM_GPU=1 DEMO_PORT=8601 VLLM_PORT=8010 \
  scripts/deploy_server.sh
```

Скрипт готовит теневой чекпойнт (восстанавливает `auto_map`, который
`prepare_checkpoint.py` вырезает), поднимает `docker/compose.server.yml` и
проверяет, что на порту отвечает именно наш vLLM с нашей моделью — на занятом
хосте чужой сервис легко ответит 404 и деплой будет выглядеть удачным.

`scripts/deploy_server.sh --down` — остановить.

Два контейнера, а не один, потому что у них разное время жизни: vLLM владеет
картой и стартует ~40 с, демка перезапускается за секунду. Выкладка нового кода
не перезагружает 6 ГБ весов, а чекаут смонтирован — не нужно пересобирать
60-гигабайтный образ.

Проверено на 2× RTX 5060 Ti (Blackwell, sm_120): статья с arxiv разбирается за
49 с, повтор — 0.32 с, простой освобождает карту (12173 → 1163 МиБ), заявка
агента будит обратно.
...
# веб-демка на http://127.0.0.1:8601, движок по умолчанию — vLLM
# (поднимает vLLM на :8000 и демку рядом одной командой)
CKPTDIR="$CKPT" scripts/run_local_vllm.sh

# та же демка, но модель грузится в сам процесс (без сервера)
...
(do_not_unload_model) растягивает это окно до `DEMO_KEEP_LOADED_IDLE_S` (3600 c),
а не навсегда — забытый чекбокс не держит GPU бесконечно;
кнопки загрузить/выгрузить — в шапке UI.
...
## Движок инференса: vLLM по умолчанию

Демка по умолчанию считает через **vLLM** (`DEMO_ENGINE=vllm`), а не загружает
модель в свой процесс. Замер на RTX 4070 Ti, те же страницы, тот же промпт,
greedy, префикс-кэш у vLLM отключён ради честности:

| | vLLM | в процессе (transformers) |
|---|---|---|
| стр. 0 (1277 ток.) | **11.98 с**, 119.6 t/s | 21.89 с, 62.0 t/s |
| стр. 3 (1684 ток.) | **15.35 с**, 119.9 t/s | 28.91 с, 61.0 t/s |
| TTFT | 1.30 с | 1.32 с (паритет) |
| 4 страницы разом | **379.8 t/s суммарно** | нет батчинга, строго по одной |

Декодирование в 1.92 раза быстрее, страница — в ~1.85. Префилл и vision-башня
одинаковы, так что разрыв целиком в цикле генерации. Против потолка по полосе
памяти (127 t/s, см. [reports/perf-findings.md](reports/perf-findings.md)) vLLM
берёт 94 %, наш путь — 49 %.

Что это означает практически:

- **Демке нужен поднятый vLLM-сервер.** `scripts/run_local_vllm.sh` поднимает
  оба процесса одной командой. Без сервера демка не молчит, а показывает ошибку
  с адресом, по которому не достучалась.
- **Кнопки «загрузить» / «выгрузить» работают по-настоящему**: картой владеет
  сервер, поэтому используется его sleep mode — замерено 9.6 → 2.3 ГиБ и обратно.
  Для этого сервер стартует с `--enable-sleep-mode` и `VLLM_SERVER_DEV_MODE=1`.
- **Очередь, артефакты, отмена задач и живые t/s одинаковы** для обоих движков —
  различается только путь инференса, поэтому их можно сравнивать напрямую.
- Всё, что ниже про attention, относится к движку `transformers`; в vLLM своя
  реализация внимания, и наши флаги на неё не влияют.

## Attention: flex_attention по умолчанию (движок transformers)
...
- [RepnikovPavel](https://github.com/RepnikovPavel) — автор и мейнтейнер проекта.
- AI-агенты (подписываются трейлером `Agent: ...` в сообщениях коммитов):
  - **Claude Fable 5** (Anthropic) — значительная часть кода демки и пайплайна;
  - **kimi (Kimi Code, Moonshot AI)** — демка: пачки скриншотов (вставка из
    буфера / drag&drop / мультивыбор), экспорт zip и pdf с типсетом формул
    (MathJax → SVG → fitz), защита от OOM на больших страницах и скриншотах;
  - **OpenCode Agent**, **ZCode (GLM-5.2)** — отдельные доработки (см. историю
    коммитов).

> Панель Contributors на GitHub заполняется только по email авторов коммитов,
> привязанным к GitHub-аккаунтам, — агенты без аккаунта там не появляются,
> поэтому список ведётся здесь.
